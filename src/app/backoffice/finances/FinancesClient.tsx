...
export default function FinancesClient() {
  ...
  const fetchHistory = async () => {
    if (!hStart || !hEnd) return alert("Selecciona Rango de Fechas");
    try {
      ...
      if (res.ok) {
        ...
        alert("Error al obtener histórico");
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const showHistoricalOrders = false; // By default history just shows totals as it can be massive, but metrics work.
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 mb-10">
          ...
          <div className="bg-[#1c1b1b] rounded-xl p-6 border-l-4 border-[#70d8c8] transition-all hover:bg-[#201f1f] shadow-xl shadow-[#70d8c8]/5">
            ...
            <p className="text-3xl font-black text-[#70d8c8]">
              S/{" "}
              {activeTab === "current"
                ? displayStats.expectedCashInRegister.toFixed(2)
            ...
            {activeTab === "current" && (
              <p className="text-[#bdc9c6] text-xs mt-2 opacity-80 flex items-center gap-1 font-['Plus_Jakarta_Sans']">
                ...
                {session.startingCash.toFixed(2)} + Venta: S/
                {displayStats.totalCash.toFixed(2)}
              </p>
            )}
          </div>
            ...
            {activeTab === "current" ? (
              ...
              <div className="bg-[#201f1f] rounded-xl border border-[#3d4947]/30 shadow-lg h-full p-8 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm text-[#bdc9c6] opacity-80">
                  Visualizando el saldo de {displayStats.orderCount} atenciones<br/>
            ...
            <section className="bg-[#201f1f] rounded-xl p-6 border border-[#3d4947]/30 shadow-lg">
              ...
              <div className="space-y-3">
                {displayStats.topDishes && displayStats.topDishes.length > 0 ? (
                  displayStats.topDishes.map((dish: any, idx: number) => (
                    <div
                      key={idx}
              ...
              <div className="bg-white text-black p-6 rounded-md font-['Courier_New',Courier,monospace] shadow-md border border-[#e5e2e1]">
                {(() => {
                  ...
                  if (selectedOrder.adjustment) {
                    adjAmount = subtotal * (selectedOrder.adjustment.percent / 100);
                    if (selectedOrder.adjustment.kind === "discount") adjAmount = -adjAmount;
                  }
                  ...
                  return (
                    <>
                      <div className="text-center mb-6 border-b border-dashed border-gray-400 pb-4">
                        <h4 className="font-bold text-xl uppercase tracking-widest">VeryFazty Resto</h4>
                        <p className="text-sm">RUC: 20123456789</p>
                        <p className="text-xs mt-2 uppercase">Ticket / Pre-Cuenta</p>
                        <p className="font-bold text-lg mt-1">N° #{selectedOrder.orderNumber}</p>
                      </div>

                      <div className="text-xs mb-4 leading-tight">
                        <p>FECHA : {new Date(selectedOrder.createdAt).toLocaleString()}</p>
                        <p>MESA  : {selectedOrder.tableNumber || "No especificada"}</p>
                        <p>ESTADO: {selectedOrder.status === "paid" ? "PAGADO" : selectedOrder.status === "cancelled" ? "ANULADO" : "PENDIENTE"}</p>
                      </div>

                      {selectedOrder.customer && selectedOrder.customer.documentType !== "none" && (
                        <div className="text-xs mb-4 border-t border-dashed border-gray-400 pt-4 leading-tight">
                          <p>CLIENTE: {selectedOrder.customer.name} {selectedOrder.customer.surname}</p>
                          <p>DOC    : {selectedOrder.customer.documentType.toUpperCase()} {selectedOrder.customer.documentNumber}</p>
                          {selectedOrder.customer.address && <p>DIR    : {selectedOrder.customer.address}</p>}
                        </div>
                      )}
                      ...
                      <table className="w-full text-xs mt-4">
                        <thead className="border-y border-dashed border-gray-400">
                          <tr>
                            ...
                            <th className="py-2 text-left font-normal">DESCRIPCIÓN</th>
                            <th className="py-2 text-right font-normal">P.UNIT</th>
                            <th className="py-2 text-right font-normal">IMPORT</th>
                          </tr>
                        </thead>
                        <tbody>
                          {selectedOrder.items?.map((item: any, idx: number) => (
                            <tr key={idx}>
                              ...
                              <td className="py-2 text-right align-top">{(item.qty * item.unitPrice).toFixed(2)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                      ...
                      <div className="border-t border-dashed border-gray-400 mt-4 pt-4 text-xs space-y-1">
                        ...
                        {selectedOrder.adjustment && (
                          <div className="flex justify-between">
                            <span>{selectedOrder.adjustment.kind === "discount" ? "DESCUENTO" : "RECARGO"} ({selectedOrder.adjustment.percent}%):</span>
                            <span>{selectedOrder.adjustment.kind === "discount" ? "-" : "+"} S/ {Math.abs(adjAmount).toFixed(2)}</span>
                          </div>
                        )}
                      ...
                      {selectedOrder.payments && selectedOrder.payments.length > 0 && selectedOrder.status === "paid" && (
                        <div className="border-t border-dashed border-gray-400 mt-4 pt-4 text-xs space-y-1">
                          ...
                          {selectedOrder.payments.map((p: any, idx: number) => {
                             ...
                             if (p.type === "transfer") typeName = "TRANSFERENCIA";
                             return (
          ...
          >
            <div className="flex flex-col items-center p-8 text-center">
              ...
              <p className="text-sm text-[#bdc9c6] opacity-80 mb-6">
                Estás a punto de anular permanentemente esta transacción. Esta acción no se puede deshacer y alterará los totales de caja.
              </p>
        ...
        >
          ...
          >
            <div className="flex flex-col items-center p-8 text-center">
              ...
              <p className="text-sm text-[#bdc9c6] opacity-80 mb-6">
                Estás a punto de congelar las ventas y dar por finalizada esta sesión. Esta acción es definitiva y no podrás revertirla. ¿Estás seguro/a?
              </p>
              <div className="flex gap-3 w-full">
                <button
                  onClick={() => setCloseSessionConfirm(false)}
